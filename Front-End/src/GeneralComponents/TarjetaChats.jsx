const TarjetaChats = ( { user_id, info, rol, navigate, route, perfilRoute } ) => (
  <article className='p-[2%] w-[80%] font-bold font-montserrat flex justify-center items-center text-black font-montserrat border rounded-3xl m-[1%]'>
    ...
    <div className='flex flex-col w-[40%] h-full justify-center items-center'>
      ...
      onClick={() => navigate(perfilRoute, { state: {id: rol == 2 && (info.id_donante || info.id_admin) || rol == 3 && (info.id_escuela || info.id_admin) || rol == 1 && (info.id_escuela || info.id_donante) }})}>
        Ver perfil
      </button>
    </div>
    <div className='w-[40%] h-[15vh] flex flex-col items-center'>
      ...
      onClick={() => navigate(route, { state: {id_chat: info.id_chat} })}>
        Entrar al chat
      </button>
    </div>
