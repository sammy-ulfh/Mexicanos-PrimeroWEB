...
const Tarjeta = ({type, navigate, data}) => (
  <article className='p-[2%] w-[80%] font-bold font-montserrat flex flex-col justify-center items-center text-black font-montserrat border rounded-3xl m-[1%]'>
    ...
    <button className='border rounded-3xl text-sm sm:text-sm md:text-base lg:text-lg xl:text-xl w-[50%] md:w-[40%] lg:w-[40%] xl:w-[40%] h-[5vh] mt-[1%] bg-[#009933] hover:scale-105 transition duration-300' onClick={() => navigate('/apoyos/info', { state: data })}>
      Ver información
    </button>
  </article>
);
            ...
            </div>
            {
              infoSupport.length > 0 ? (
                infoSupport.map((tarjeta) => (
                  <Tarjeta key={id_apoyo} type={tarjeta.tipo_apoyo_escuela} navigate={navigate} data={tarjeta}  />
                ))
              ) : (
